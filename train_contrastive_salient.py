...
from src.contrastive_tools import (
    ...
    SupConLoss,
    fit_salient_contrastive,
    validate_salient_contrastive,
)
from src.torch_tools import set_seeds
...
model_name = f"vae_ld-{model_params['latent_dim']}_contrastive_salient-{training_params['salient_dim']}_rs-{training_params['seed']}"
output_directory = f"../../data/ltsaot6/models/contrastive/ld{latent_dim}/{model_name}"
...
# usual way
model = CNN_ContrastiveVAE(scaler=StandardScaler(), **model_params)
model.to(device)
...
for i in range(training_params["max_epochs"]):
    ...
    train_loss, train_mse, train_kl, train_contrastive = fit_salient_contrastive(
        model=model,
        criterion=criterion,
    ...
    test_loss, test_mse, test_kl, test_contrastive = validate_salient_contrastive(
        model=model,
        criterion=criterion,
        sup_con_loss=contrastive_loss,
    ...
    if test_loss < best_loss:
        ...
        print(
            f"Epoch {i}, Train loss {train_loss:.6f}, mse {train_mse:.6f}, kl {train_kl:.10f}, cont {train_contrastive:.6f} // Test loss {test_loss:.6f}, mse {test_mse:.6f}, kl : {test_kl:.10f}, cont {test_contrastive:.6f}; time : {int(time_epoch)}s, best epoch "
        )
    else:
        nb_stag += 1
        print(
            f"Epoch {i}, Train loss {train_loss:.6f}, mse {train_mse:.6f}, kl {train_kl:.10f}, cont {train_contrastive:.6f} // Test loss {test_loss:.6f}, mse {test_mse:.6f}, kl : {test_kl:.10f}, cont {test_contrastive:.6f}"
        )
